...
const CdekSpinner = /* @__PURE__ */ _export_sfc(_sfc_main$o, [["render", _sfc_render], ["__scopeId", "data-v-c4a93e36"]]);
...
class CdekApi extends AbstractApi {
  ...
  async getOffices() {
    ...
    const offices = await this.fetchOfficePage(1, additionalParams, 1).then((r) => {
      if (!(r.headers instanceof AxiosHeaders2) || !r.headers.has("x-total-elements")) {
        return Promise.all([r.data]);
      }
      return Promise.all(
        Array.from(
          {
          ...
          (_e, i) => this.fetchOfficePage(i, additionalParams, officesPageSize).then((r2) => r2.data)
        )
      );
  ...
  async fetchOfficePage(pageNumber, additionalParams, sizeNumber = 500) {
      ...
      ({
        ...
        after(() => {
          ...
          if (store2.uiLoadLocksCounter > 0)
            return;
          if (store2.debug) {
            console.debug("[CDEK] Widget 3.9.4 has been loaded");
          }
