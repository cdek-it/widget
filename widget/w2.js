...
function ISDEKWidjet(params) {
    ...
    var DATA = {
        ...
        PVZ: {
            ...
            getByCoord: function (data) {
                console.log('getByCoord=> timer', data);
                console.log('DATA.city.current=', DATA.city.current);
                console.log('DATA.PVZ.bycoordCur=', DATA.PVZ.bycoordCur);
                console.log('!=', DATA.city.current!=DATA.PVZ.bycoordCur);
                console.log('!=', DATA.city.current!=DATA.PVZ.bycoordCur);
                var time = performance.now();
                if (!Object.keys(DATA.PVZ.bycoord).length || DATA.city.current!=DATA.PVZ.bycoordCur)
                    DATA.PVZ.bycoord = this.checkByCoord(data);

                time = performance.now() - time;
                console.log('Время выполнения = ', time);

                return DATA.PVZ.bycoord;

            },

            checkByCoord: function (data) {
                // data = [[54.85474218774078, 82.5927827163085], [55.09173014928511, 83.46138928369135]];
                console.log('checkByCoord=> timer');
                DATA.PVZ.bycoordCur = DATA.city.current;
                var time = performance.now();
                var listPVZ = {};
                for (var pvzCity in DATA.PVZ.collection) {
                    for (var myPvz in DATA.PVZ.collection[pvzCity]) {
                        ...
                        if (mpvz.cY > data[0][0] &&  mpvz.cY < data[1][0]
                            && mpvz.cX > data[0][1] && mpvz.cX < data[1][1] ) listPVZ[myPvz]=mpvz;
                    }
                }
                time = performance.now() - time;
                console.log('Время выполнения = ', time);
    ...
    var template = {
        ...
        controller: {
            ...
            loadCity: function (doLoad) {
                ...
                this.updatePrices();
                console.log('in loadCity DATA.city.current',DATA.city.current);
            ...
            choosePVZ: function (id) {
                ...
                // var PVZ = DATA.PVZ.getByCoord();
                var PVZ = (widjet.options.get('bymapcoord'))?  DATA.PVZ.getByCoord(this.map.getBounds()) : DATA.PVZ.getCurrent();
        ...
        ymaps: {
            ...
            loadMap: function (city) {
                ...
                ymaps.geocode(city, {
                    ...
                    if (!self.map) {
                        console.log('NOT self.map');
                        console.log('self.linker',self.linker);
                        ...
                        console.log('NOT self.map step 2');
                        ...
                        console.log('NOT self.map step 3');
                        self.map.events.add('boundschange', widjet.hideLoader);
                        console.log('end NOT self.map');
                    } else {
                        console.log('ELSE NOT self.map');
            ...
            placeMarks: function (mtypes) {
                // var pvzList = DATA.PVZ.getByCoord();
                // console.log('placeMarks=>pvzList', map.geoObjects.getBounds());
                // console.log(pvzList);
                var self = this;
                ...
                if(!this.map) {
                    this.map = new ymaps.Map(template.ymaps.linker, {
                        zoom: 10,
                        controls: [],
                        center: [53.371174, 83.720080]
                    });
                }
                console.log('this.map', this.map);
                console.log('bymapcoord', widjet.options.get('bymapcoord'));
                console.log('placeMarks map.getBounds', this.map.getBounds());

                // var pvzList =  DATA.PVZ.getCurrent();
                var pvzList = (widjet.options.get('bymapcoord'))?  DATA.PVZ.getByCoord(this.map.getBounds()) : DATA.PVZ.getCurrent();
                console.log('pvzList', pvzList);
            ...
            selectMark: function (wat) {
                ...
                // var cityPvz = DATA.PVZ.getByCoord();
                var cityPvz = (widjet.options.get('bymapcoord'))?  DATA.PVZ.getByCoord(this.map.getBounds()) : DATA.PVZ.getCurrent();
                console.log('wat', cityPvz[wat]);
                console.log('CityCode', cityPvz[wat].CityCode);
                console.log('DATA.city.current', DATA.city.current);
                if (cityPvz[wat].CityCode != DATA.city.current) {
                    // DATA.city.set(cityPvz[wat].CityCode);
                    template.controller.selectCity(cityPvz[wat].CityCode);
                    // this.loadMap(cityPvz[wat].CityCode);
                } else {
